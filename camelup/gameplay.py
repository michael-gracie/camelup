...
class GamePlay():
    ...
    def _move(self):
        ...
        if self.temp_dict[self.camel]['space'] in self.tiles.keys():
            if self.tiles[self.temp_dict[self.camel]['space']] == 'block':
                ...
                self._sourceHeight()
                log.info('Blocked: camel changes are the following:' + str(
                    self.temp_dict))
                ...
                self._destHeight()
                self._sourceHeight()
                log.info('Skipped: camel changes are the following:' + str(
                    self.temp_dict))
        else:
            self._destHeight()
            self._sourceHeight()
        ...
        for key, val in self.camel_dict.items():
            if key not in self.temp_dict:
                if val['space'] == self.temp_dict[self.camel]['space']:
                    self.temp_dict[key]['height'] = self.camel_dict[key][
                        'height'] + self.camel_dict[self.camel]['height']
    ...
    def _sourceHeight(self):
        ...
        for key, val in self.camel_dict.items():
            if key not in self.temp_dict:
                if val['space'] == self.camel_dict[self.camel][
                    'space'] and val['height'] > self.camel_dict[self.camel]['space']:
                    self.temp_dict[key]['height'] = self.camel_dict[key][
                        'height'] - self.camel_dict[self.camel]['height']
    ...
    def _destHeightBlock(self):
        ...
        camel hits a block
        """
        heights = [val['height'] for key, val in self.camel_dict.items(
            ) if val['space'] == self.temp_dict[
            self.camel]['space'] and key not in self.temp_dict.keys()]
        ...
        for key, val in self.temp_dict.items():
            if heights:
                if val['space']:
                    self.temp_dict[key]['height'] = max(heights) + self.camel_dict[
                        key]['height']
...
def camels_to_move(camel_df, space, height):
    '''
    '''
    return camel_df.loc[camel_df['space'].eq(space) &
                camel_df['height'].gt(height - 0.000001)].index

def camels_in_dest(camel_df, destination):
    '''
    Returns camels in destination
    '''
    return camel_df[camel_df['space'].eq(space)].index
...
def block_move(camel_df, movers, destination):
    ...
    if dest_camels:
        ...
        max_height_dest = 0
    camel_df.loc[movers]['space'] = destination
    camel_df.loc[movers]['height'] = camel_df.loc[movers]['height'] - (min_height_movers - 1)
    if dest_camels:
        camel_df.loc[dest_camels]['height'] = camel_df.loc[dest_camels]['height'] + num_movers
